...
INSERT INTO profiles (name, secret, organization) VALUES ($1, $2, $3) RETURNING id
`
...
func (q *Queries) AddProfile(ctx context.Context, arg AddProfileParams) (int32, error) {
	row := q.db.QueryRowContext(ctx, addProfile, arg.Profilename, arg.Secret, arg.Orgname)
	var id int32
	err := row.Scan(&id)
	return id, err
}
...
SELECT id, name, secret, organization FROM profiles WHERE lower(organization) = lower($1)
`
...
SELECT id, name, secret, organization FROM profiles WHERE lower(organization) = lower($1) and secret = FALSE
`
...
SELECT id, name, secret, organization FROM profiles WHERE lower(name) = $1 AND lower(organization) = lower($2)
`
...
SELECT id, name, secret, organization FROM profiles
`
