...
const addExerciseDb = `-- name: AddExerciseDb :exec
INSERT INTO Exercise_dbs (name, organization, url, sign_key, auth_key, tls) VALUES ($1, $2, $3, $4, $5, $6)
`

type AddExerciseDbParams struct {
	Exdbname string `json:"exdbname"`
	Org      string `json:"org"`
	Url      string `json:"url"`
...
func (q *Queries) AddExerciseDb(ctx context.Context, arg AddExerciseDbParams) error {
	_, err := q.db.ExecContext(ctx, addExerciseDb,
		arg.Exdbname,
		arg.Org,
		arg.Url,
		arg.SignKey,
...
const checkIfExDbExists = `-- name: CheckIfExDbExists :one
SELECT EXISTS( SELECT 1 FROM Exercise_dbs WHERE lower(name) = lower($1) )
`

func (q *Queries) CheckIfExDbExists(ctx context.Context, exdbname string) (bool, error) {
	row := q.db.QueryRowContext(ctx, checkIfExDbExists, exdbname)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
SELECT id, name, organization, url, sign_key, auth_key, tls FROM Exercise_dbs
`
